...
fn adapter_labels_every_legacy_edge_as_heuristic() {
    ...
    assert!(analysis.relationships.iter().all(|edge| relationship_origin(edge) == "heuristic"));

    let persists_evidence: Vec<_> = analysis.evidence.iter().filter_map(|evidence| match evidence {
        Evidence::Analyzer { origin, confidence, rule, .. } if rule.as_deref() == Some("model_name_match") => Some((origin, confidence)),
        _ => None,
    }).collect();
...
fn traversal_claim_has_sets_without_order_or_steps() {
    ...
    let inventory = Inventory::discover(&root, &Config::default()).unwrap();
    let analysis = heuristic::analyze(&root, "fixture/heuristic", &inventory, &Config::default());
    for claim in analysis.claims.iter().filter(|claim| claim.predicate == "HeuristicTraversal") {
