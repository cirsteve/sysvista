...
The TypeScript analyzer is bundled by esbuild into one `dist/analyzer.js`. The Rust build requires that bundle and runs `npm run build` when it is absent; dependencies must already be installed with `npm ci` (or via `make build-analyzer`). It copies the bundle into `OUT_DIR`, embeds it with `include_bytes!`, and at runtime atomically extracts it into a private, user-owned cache directory as `analyzer-<version>.js`. It invokes `node` from `PATH`; it never installs packages or runs repository scripts at scan time.
