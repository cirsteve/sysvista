...
The completed analyzer bundle is 9,955,583 bytes. In the local debug measurement the CLI grew from the pre-analyzer 56,429,000 bytes to 67,588,920 bytes, an 11,159,920-byte delta (debug metadata makes that larger than the embedded payload). Release/platform measurements should be refreshed when TypeScript or esbuild changes.
