...
The handshake-spike bundle is 947 bytes. In the initial local debug measurement the CLI grew from 56,429,000 bytes to 57,404,240 bytes, a 975,240-byte delta (debug metadata makes that larger than the embedded payload). Release/platform measurements should be refreshed when TypeScript or esbuild changes.
