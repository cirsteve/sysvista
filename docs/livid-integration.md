...
The viewer selects stable entity and relationship IDs from one scope at a time. Structural navigation uses `index/scopes.json` children and owner mappings; crossing relationships are aggregated to visible owners. Function-scoped entities marked `is_local` remain in the data but are hidden in the default child selection. The flow lens starts from the selected entity and expands to a bounded hop count. `adapter.ts` turns each selected slice into a Livid spec and `selection.ts` maps renderer selection back to SysVista IDs.
