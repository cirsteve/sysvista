...
A one-time [GitHub Actions dispatch](https://github.com/cirsteve/sysvista/actions/runs/35933583282) on PR C head `d5351ac` confirmed that the runner has `unzip`: it extracted and loaded the source archives. The job failed at the first expected-rejection check because `rg` is absent on that runner. The gate now uses `grep -Ei` for that check; a clean-install local rerun passed all gate stages, including all three expected rejections. This dispatch is preliminary runner evidence, not a successful integration-head gate result; the integration-head rerun remains pending the merge.
